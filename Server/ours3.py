...
class Ours3(ServerMethod):
    """
    Variant: use fc1.weight + fc2.weight layers for FINCH clustering.
    """
    NAME = 'Ours3'

    def __init__(self, args, cfg):
        super(Ours3, self).__init__(args, cfg)
    ...
    def server_update(self, **kwargs):
        ...
        # ===== FINCH clustering: use fc1.weight + fc2.weight =====
        fc1_slice = None
        fc2_slice = None
        for (layer_name, start, end) in param_info:
            if layer_name == "feats.fc1.weight" or layer_name.endswith("fc1.weight"):
                fc1_slice = (start, end)
            if layer_name == "feats.fc2.weight" or layer_name.endswith("fc2.weight"):
                fc2_slice = (start, end)
            if fc1_slice and fc2_slice:
                break

        div_score = []
        for query_index, _ in enumerate(nets_list):
            if fc1_slice is None and fc2_slice is None:
                client_vec = weight_grad_list[query_index].view(1, -1)
                global_vec = weight_global_grad.view(1, -1)
            else:
                parts_client = []
                parts_global = []
                if fc1_slice is not None:
                    s, e = fc1_slice
                    parts_client.append(weight_grad_list[query_index][s:e])
                    parts_global.append(weight_global_grad[s:e])
                if fc2_slice is not None:
                    s, e = fc2_slice
                    parts_client.append(weight_grad_list[query_index][s:e])
                    parts_global.append(weight_global_grad[s:e])
                client_vec = torch.cat(parts_client).view(1, -1)
                global_vec = torch.cat(parts_global).view(1, -1)
