...
# On Windows, multiprocessing DataLoader with lambda transforms can fail pickling;
# use single-process loading to avoid spawn-pickle issues.
    ...
    def partition_label_skew_loaders(self, train_dataset, test_dataset) -> Tuple[list, DataLoader, dict]:
        ...
        for j in range(n_participants):
            ...
            train_loader = DataLoader(train_dataset,
                                      batch_size=self.cfg.OPTIMIZER.local_train_batch, sampler=train_sampler, num_workers=0, drop_last=True)
            self.train_loaders.append(train_loader)

        self.val_loader = DataLoader(train_dataset,batch_size=self.cfg.OPTIMIZER.local_train_batch,
        sampler=SubsetRandomSampler(val_idx),num_workers=0,drop_last=True)

        test_loader = DataLoader(test_dataset,
                                 batch_size=self.cfg.OPTIMIZER.local_test_batch, shuffle=False, num_workers=0)
